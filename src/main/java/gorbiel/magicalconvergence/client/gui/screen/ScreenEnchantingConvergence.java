...
public class ScreenEnchantingConvergence extends AbstractContainerScreen<ContainerEnchantingConvergence> {
	...
	private static final ResourceLocation ENCHANTMENT_TABLE_GUI_TEXTURE =
			ResourceLocation.fromNamespaceAndPath(EnchantingConvergence.MOD_ID, "textures/gui/container/enchanting_table.png");
	...
	public ScreenEnchantingConvergence(ContainerEnchantingConvergence menu, Inventory playerInventory, Component title) {
	...
	public boolean mouseClicked(double mouseX, double mouseY, int button) {
		...
		for (int i = 0; i < this.enchantButtons; i++) {
			...
			if (this.enchantingMode == EnchantingMode.NORMAL) {
				...
				this.minecraft.gameMode.handleInventoryButtonClick(this.menu.containerId, (1 << 31) | (this.scrollStartIndex + i));
				return true;
			}
		}
...
//		graphics.blit(ENCHANTMENT_TABLE_GUI_TEXTURE, this.leftPos, this.topPos, 0, 0, this.imageWidth, this.imageHeight);
		graphics.blit(ENCHANTMENT_TABLE_GUI_TEXTURE, this.leftPos, this.topPos, 0, 0, this.imageWidth, this.imageHeight, 512, 256);
		...
		int offset = scrollbarEnabled
				? (int) ((double) this.scrollStartIndex / (double) Math.max(this.menu.getEnchantmentList().size() - this.enchantButtons, 1) * 55.0D)
				: 0;

//		graphics.blit(ENCHANTMENT_TABLE_GUI_TEXTURE, this.leftPos + 261, this.topPos + 14 + offset, 219 + (scrollbarEnabled ? 0 : 6), 187, 6, 21);
		graphics.blit(ENCHANTMENT_TABLE_GUI_TEXTURE, this.leftPos + 261, this.topPos + 14 + offset, 219 + (scrollbarEnabled ? 0 : 6), 187, 6, 21, 512, 256);
...
//					graphics.blit(ENCHANTMENT_TABLE_GUI_TEXTURE, x1, y1, 0, 206, 200, 19);
				...
				boolean selected = this.isHovering(ROW_X, ROW_Y + ROW_SPACING * i, ROW_WIDTH, ROW_HEIGHT, mouseX, mouseY);

				if (!creative && enchLevel < ench.getMaxLevel() && !UnlockedEnchantmentHelper.isUnlocked(this.minecraft.player, ench, enchLevel + 1)) {
//					graphics.blit(ENCHANTMENT_TABLE_GUI_TEXTURE, x1, y1, 0, 206, 200, 19);
					graphics.blit(ENCHANTMENT_TABLE_GUI_TEXTURE, x1, y1, 0, 206, 200, 19, 512, 256);
//					graphics.drawString(this.font, this.getDisplayName(ench), x1 + 6, y1 + 6, 0x342F25, false);
...
//				graphics.blit(ENCHANTMENT_TABLE_GUI_TEXTURE, x1, y1, 0, selected ? 225 : 187, 200, 19);
				graphics.blit(ENCHANTMENT_TABLE_GUI_TEXTURE, x1, y1, 0, selected ? 225 : 187, 200, 19, 512, 256);

				if (enchLevel >= ench.getMaxLevel()) {
//					graphics.drawString(this.font, this.getDisplayName(ench).copy().append(" MAX"), x1 + 6, y1 + 6, selected ? 0xFFFF80 : 0x685E4A, false);
					Component text = this.getDisplayName(ench).copy().append(" MAX")
							.withStyle(style -> style.withColor(selected ? 0xFFFF80 : 0x685E4A));
					graphics.drawString(this.font, text, x1 + 6, y1 + 6, 0, false);
					continue;
				}

				if (!enchantments.containsKey(ench) && !EnchantmentHelper.isEnchantmentCompatible(enchantments.keySet(), ench)) {
//					graphics.drawString(this.font, this.getDisplayName(ench).copy().withStyle(ChatFormatting.RED, ChatFormatting.STRIKETHROUGH), x1 + 6, y1 + 6, selected ? 0xFFFF80 : 0x685E4A, false);
					Component text = this.getDisplayName(ench).copy().withStyle(ChatFormatting.RED, ChatFormatting.STRIKETHROUGH);
					graphics.drawString(this.font, text, x1 + 6, y1 + 6, 0, false);
					continue;
				}
...
//				graphics.blit(ENCHANTMENT_TABLE_GUI_TEXTURE, x1 + 143, y1, 200, 187, 19, 19);
				...
				graphics.drawString(this.font, requiredLevels, x1 + 143 + 18 - this.font.width(requiredLevels), y1 + 10, enoughLevels ? 0xF0F0F0 : 0xA50000, false);

				String requiredLapis = Integer.toString(lapisCost);
				graphics.drawString(this.font, requiredLapis, x1 + 162 + 18 - this.font.width(requiredLapis), y1 + 10, enoughLapis ? 0xF0F0F0 : 0xA50000, false);

				String requiredPower = Integer.toString(powerCost);
				graphics.drawString(this.font, requiredPower, x1 + 181 + 18 - this.font.width(requiredPower), y1 + 10, enoughPower ? 0xF0F0F0 : 0xA50000, false);

				graphics.pose().popPose();

//				graphics.drawString(this.font, ench.getFullname(enchLevel + 1), x1 + 6, y1 + 6, selected ? 0xFFFF80 : 0x685E4A, false);
				Component text = ench.getFullname(enchLevel + 1).copy()
						.withStyle(style -> style.withColor(selected ? 0xFFFF80 : 0x685E4A));
				graphics.drawString(this.font, text, x1 + 6, y1 + 6, 0, false);
...
//				graphics.blit(ENCHANTMENT_TABLE_GUI_TEXTURE, x1 + 143, y1, 200, 187, 19, 19);
	...
	private void renderBook(GuiGraphics graphics, int leftPos, int topPos, float partialTick) {
		...
		VertexConsumer vc = graphics.bufferSource().getBuffer(this.bookModel.renderType(ENCHANTMENT_TABLE_BOOK_TEXTURE));
...
//
//				if (!creative && !UnlockedEnchantmentHelper.isUnlocked(this.minecraft.player, ench, enchLevel)) {
//					tooltip.add(Component.literal("Locked").withStyle(ChatFormatting.RED));
//				} else {
//					tooltip.add(Component.literal("XP: " + levelCost).withStyle(this.minecraft.player.experienceLevel >= levelCost || creative ? ChatFormatting.GRAY : ChatFormatting.RED));
//					tooltip.add(Component.literal("Lapis: " + lapisCost).withStyle(this.menu.getLapisAmount() >= lapisCost || creative ? ChatFormatting.GRAY : ChatFormatting.RED));
//					tooltip.add(Component.literal("Power: " + powerCost).withStyle(this.menu.getEnchantingPower() >= powerCost || creative ? ChatFormatting.GRAY : ChatFormatting.RED));
//				}
//			} else {
//				int levelCost = (i + 1) * 10;
//				int lapisCost = (i + 1) * 5;
//				int powerCost = i * 4 + 3;
//
//				tooltip.add(Component.literal("Unlock random enchantment").withStyle(ChatFormatting.WHITE));
//				tooltip.add(Component.empty());
//				tooltip.add(Component.literal("XP: " + levelCost).withStyle(this.minecraft.player.experienceLevel >= levelCost || creative ? ChatFormatting.GRAY : ChatFormatting.RED));
//				tooltip.add(Component.literal("Lapis: " + lapisCost).withStyle(this.menu.getLapisAmount() >= lapisCost || creative ? ChatFormatting.GRAY : ChatFormatting.RED));
//				tooltip.add(Component.literal("Power: " + powerCost).withStyle(this.menu.getEnchantingPower() >= powerCost || creative ? ChatFormatting.GRAY : ChatFormatting.RED));
